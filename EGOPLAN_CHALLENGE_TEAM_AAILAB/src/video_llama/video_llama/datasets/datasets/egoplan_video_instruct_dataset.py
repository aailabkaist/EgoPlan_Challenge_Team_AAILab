        ...
        for _ in range(num_retries):
            try:
                ...
                if self.answer_type == "egoplan_qa":
                    ...
                    candidate_questions = [
                        "What exact actions were depicted in the video? Please list them in order, describing each action with a brief verb-noun phrase.",
                    ...
                    if 'add_narr' in sample.keys():
                        narr = sample['add_narr']
                        question = narr + " " + question
                ...
                image, clips, clip_mask, _ = self.vis_processor(sample)
...
###########################################################
class Egoplan_Video_Instruct_Bound_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root,
                 ann_root, num_video_query_token=32,
                 tokenizer_name='/mnt/workspace/ckpt/vicuna-13b/', data_type='video',
                 model_type='vicuna', n_actions=4, answer_type='egoplan_qa'):
...
class Egoplan_Video_Instruct_DPO_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root,
                 ann_root, num_video_query_token=32,
                 tokenizer_name='/mnt/workspace/ckpt/vicuna-13b/', data_type='video',
                 model_type='vicuna', n_actions=4, answer_type='egoplan_qa'):
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                candidate_questions = [
                    "What exact action was most recently depicted in the video? Please describe the action with a brief verb-noun phrase.",
                    "Can you enumerate the most recent action in the video, describing it with a short verb-noun combination?",
                    "Could you break down the most recent action from the video? Use a succinct verb-noun pair.",
                    "Can you dissect the most recent action from the video's content? Please use a verb-noun pair.",
                    "Can you detail the most recent action shown in the video using a short verb-noun description?"]
                ...
                most_recent_actions_metadata = list(
                    filter(lambda item: item['stop_frame'] <= current_observation_frame_idx,
                           task_progress_metadata))[-self.n_actions:]
                # most_recent_actions_metadata = task_progress_metadata[-self.n_actions:]
                if len(most_recent_actions_metadata) > 0:
                    answer = most_recent_actions_metadata[-1]['narration_text']
                    bool_shuffle = False
                    for i in range(100):
                        negative_answer = random.choice(sample["negative_answers"])
                        if answer != negative_answer:
                            bool_shuffle = True
                            break
                    if not bool_shuffle:
                        print(negative_answer)
                        negative_answer = "No meaningful action occurred in the video."
                else:
                    answer = "No meaningful action occurred in the video."
                    negative_answer = "A meaningful action occurred in the video."

                # print("q: "+question+"\n")
                # print("a: "+answer+"\n\n")
                # print("na: "+negative_answer+"\n\n")
    ...
    def collater(self, instances):
        ...
        batch['conv_type'] = 'egoplan_current_action_recog_dpo'
        return batch


class Egoplan_Separate_Video_Instruct_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root,
                 ann_root, num_video_query_token=32,
                 tokenizer_name='/mnt/workspace/ckpt/vicuna-13b/', data_type='video',
                 model_type='vicuna', n_actions=4, answer_type='egoplan_qa'):
        ...
        for _ in range(num_retries):
            try:
                ...
                if self.answer_type == "egoplan_qa":
                    ...
                    candidate_questions = [
                        "What exact actions were depicted in the video? Please describe the {}most recent action with a brief verb-noun phrase.",
                        "Can you enumerate the {}most recent action in the video, describing it with a short verb-noun combination?",
                        "Could you break down the {}most recent action from the video? Use a succinct verb-noun pair.",
                        "Can you dissect the {}most recent action from the video's content? Please use a verb-noun pair.",
                        "Can you detail the {}most recent action shown in the video using a short verb-noun description?"]
                    ...
                    # observed_actions = []
                    if len(most_recent_actions_metadata) > 0:
                        target_action_idx = random.randint(0, len(most_recent_actions_metadata)-1)
                        answer = most_recent_actions_metadata[-target_action_idx-1]["narration_text"]
                        if target_action_idx == 0:
                            question = question.format('')
                        elif target_action_idx == 1:
                            question = question.format('second ')
                        elif target_action_idx == 2:
                            question = question.format('third ')
                        elif target_action_idx == 3:
                            question = question.format('fourth ')
                        else:
                            raise NotImplementedError(f'target action idx error: {target_action_idx}')
                    else:
                        raise NotImplementedError('zero action')
...
class Egoplan_Video_Instruct_Dataset_v3(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root,
                 ann_root, num_video_query_token=32,
                 tokenizer_name='/mnt/workspace/ckpt/vicuna-13b/', data_type='video',
                 model_type='vicuna', n_actions=4, answer_type='egoplan_qa'):
        ...
        for _ in range(num_retries):
            try:
                ...
                if self.answer_type == "egoplan_qa":
                    ...
                    candidate_questions = [
                        "What exact actions were depicted in the video? Please describe the action with a brief verb-noun phrase.",
                        "Can you enumerate the action in the video, describing it with a short verb-noun combination?",
                        "Could you break down the action from the video? Use a succinct verb-noun pair.",
                        "Can you dissect the action from the video's content? Please use a verb-noun pair.",
                        "Can you detail the action shown in the video using a short verb-noun description?"]
                    question = random.choice(candidate_questions)
                    answer = sample['answer']
...
class Egoplan_Video_Instruct_Semantic_Matching_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root,
                 ann_root, num_video_query_token=32,
                 tokenizer_name='/mnt/workspace/ckpt/vicuna-13b/', data_type='video',
                 model_type='vicuna', n_actions=4, answer_type='egoplan_qa'):
    ...
    def collater(self, instances):
        ...
        for k in ['image', 'clips', 'clip_mask']:
            ...
            batch[k] = torch.stack(values)
            # print(f"{k}: {batch[k].shape}")
        for k in ['raw', 'positive_answer', 'negative_answer']:
            values = [instance[k] for instance in instances]
            batch[k] = values
            # print(f"{k}: {batch[k].shape}")

        batch['conv_type'] = 'egoplan_semantic_matching_v3'
        return batch
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                if 'add_narr' in sample.keys():
                    narr = sample['add_narr']
                    question = create_question_for_egoplan_task_planning_add_narr(task_goal=task_goal, narr=narr,
                                                                                  mode='train')
                else:
                    question = create_question_for_egoplan_task_planning(task_goal=task_goal,
                                                                         mode='train')
                answer = sample['answer']
                # print(type(self.vis_processor)) <class 'src.video_llama.video_llama.processors.egoplan_video_processor.EgoplanVideoTrainProcessor'>
                image, clips, clip_mask, _ = self.vis_processor(sample) # error source
                # print('#########################')
                data_dict = self.get_llm_input_ids_and_labels(question, answer)
                # print('##################### \n')
                ...
                negative_answer = random.choice(negative_answers)
                # print('######################## \n')
                negative_data_dict = self.get_llm_input_ids_and_labels(question, negative_answer) # error source
                # print(f"question: {question}\n"
                #       f"positive_answer: {answer}\n"
                #       f"negative_answer: {negative_answer}")
...
#######################################################
class Egoplan_Video_Contrastive_Dataset_Bound(Egoplan_Video_Instruct_Bound_Dataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def collater(self, instances):
        ...
        return batch
#######################################################
class Egoplan_Video_Contrastive_Dataset_MDPO_Video(Egoplan_Video_Instruct_Dataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                # print(type(self.vis_processor)) <class 'src.video_llama.video_llama.processors.egoplan_video_processor.EgoplanVideoTrainProcessor'>
                image, clips, clip_mask, _ = self.vis_processor(sample) # error source
                neg_image, neg_clips, neg_clip_mask, _ = self.vis_processor(sample, negative=True)
                data_dict = self.get_llm_input_ids_and_labels(question, answer)
                negative_answers = sample["negative_answers"]
                negative_answer = random.choice(negative_answers)
                negative_data_dict = self.get_llm_input_ids_and_labels(question, negative_answer) # error source
                ...
                data_dict['negative_labels'] = negative_data_dict["labels"]
                # Added
                data_dict['negative_image'] = neg_image
                data_dict['negative_clips'] = neg_clips
                data_dict['negative_clip_mask'] = neg_clip_mask
        ...
        return {
            ...
            "negative_text_input": data_dict["negative_input_ids"],
            "negative_labels": data_dict["negative_labels"],
            "negative_image": data_dict['negative_image'],
            "negative_clips": data_dict['negative_clips'],
            "negative_clip_mask": data_dict['negative_clip_mask'],
            "type": 'video',
        }
    # [{
    #         'image': data_dict['image'],
    ...
    #     },
    #         {
    #         'image': data_dict_negative['image'],
    #         'clips': data_dict_negative['clips'],
    #         'clip_mask': data_dict_negative['clip_mask'],
    #         "text_input": data_dict_negative["input_ids"],
    #         "labels": data_dict_negative["labels"],
    #         "negative_text_input": data_dict_negative["negative_input_ids"],
    #         "negative_labels": data_dict_negative["negative_labels"],
    #         "type": 'video',
    #     }
    #     ]
    ...
    def collater(self, instances):
        ...
        for k in ['negative_image', 'negative_clips', 'negative_clip_mask']:
            values = [instance[k] for instance in instances]
            batch[k] = torch.stack(values)
...
#######################################################
class Egoplan_Video_Contrastive_Dataset_KH(Egoplan_Video_Instruct_Dataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                # sample negative_answer
                negative_answers = sample["negative_answers"]
                negative_similarity = torch.tensor(sample['similarity'])
                # Softmax
                # softmax_similarity = torch.nn.functional.softmax(negative_similarity, dim=0)
                # negative_answer_index = torch.multinomial(softmax_similarity, 1).item()
                # Gumbel Softmax
                gumbel_samples = -torch.log(-torch.log(torch.rand_like(negative_similarity)))
                gumbel_scores = negative_similarity + gumbel_samples
                negative_answer_index = torch.argmax(gumbel_scores).item()
                negative_answer = negative_answers[negative_answer_index]
                # print('######################## \n')
                negative_data_dict = self.get_llm_input_ids_and_labels(question, negative_answer) # error source
                # print(f"question: {question}\n"
                #       f"positive_answer: {answer}\n"
...
def convert_source_vicuna_format_for_action_recognition(sources):
    new_sources = []
    for source in sources:
        new_source = []
        for i, sentence in enumerate(source):
...
def preprocess_for_llama_v2_for_action_recognition(
        sources: Sequence[str],
        tokenizer: transformers.PreTrainedTokenizer,
) -> Dict:
    """
...
prompt_list_with_narr = [
    [
        ...
        "{}"
        "Kindly offer a concise suggestion. ",
    ],
...
def create_question_for_egoplan_task_planning(task_goal, mode='train'):
    ...
    return question
#############################################################################
def create_question_for_egoplan_task_planning_choice(task_goal, mode='train', choice=0):
    task_goal = task_goal.strip(string.punctuation + " ").lower()
    if "goal" in task_goal:
        task_goal = task_goal.split("to", 1)[1].strip()
    words = task_goal.split()
    ...
    if mode == 'train':
        ...
        if words[0].endswith("ing"):
            question_pattern = prompt_list[0][choice]
            # question_pattern = "I am tasked with {}. " \
            #                    "The task's progress is demonstrated in the provided video. " \
            #                    "My current field of view is shown in the provided image. " \
            #                    "What should be my next action? " \
            ...
            question_pattern = prompt_list[1][choice]
            # question_pattern = "My current task is to {}. " \
            #                    "The task's progress is demonstrated in the provided video. " \
            #                    "My current field of view is shown in the provided image. " \
            #                    "What should be my next action? " \
    ...
    return question
#############################################################################
def create_question_for_egoplan_task_planning_bound(task_goal, mode='train'):
    task_goal = task_goal.strip(string.punctuation + " ").lower()
    if "goal" in task_goal:
        task_goal = task_goal.split("to", 1)[1].strip()
    words = task_goal.split()
    ...
    if mode == 'train':
        ...
        if words[0].endswith("ing"):
            question_pattern = "I am tasked with {}. " \
                               "The task's progress is demonstrated in the provided video. " \
                               "My current field of view is shown in the provided image. " \
                               "In the provided image, if present, a blue box indicates the left hand area, a red box the right hand area, and a yellow box the object held by each hand." \
                               "Please classify the type of contact for each bounding box in the image using the following labels: L (left hand), R (right hand), N (no contact), S (self contact), O (other person contact), P (portable object contact), F (stationary object contact, e.g., furniture) if there is a bounding box." \
                               "What should be my next action? " \
                               "Please output the most reasonable action you think, expressed in a short phrase."
        else:
            question_pattern = "My current task is to {}. " \
...
def create_question_for_egoplan_task_planning_add_narr(task_goal, narr=None, mode='train'):
    task_goal = task_goal.strip(string.punctuation + " ").lower()
    if "goal" in task_goal:
        task_goal = task_goal.split("to", 1)[1].strip()
    words = task_goal.split()
    ...
    if mode == 'train':
        if words[0].endswith("ing"):
            if narr is not None:
                question_pattern = random.choice(prompt_list_with_narr[0])
            else:
                question_pattern = random.choice(prompt_list[0])
        else:
            if narr is not None:
    ...
    if narr is not None:
        question = question_pattern.format(task_goal, narr)
    else:
        question = question_pattern.format(task_goal)
