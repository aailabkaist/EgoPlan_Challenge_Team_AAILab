...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_ver', type=str, default=None) # model version {orig/finetune}_{dataset}
    parser.add_argument('--save_name', type=str, default='') # model save name
    args = parser.parse_args()
    print('model is ', args.model)

    print(f'evaluating.. {args.model}')
    predict_choice = build(args.model, model_ver=args.model_ver)
    with open(f"accuracy/accuracy_for_{args.model}_{args.save_name}.txt", "w") as fo:
        total_correct_num = 0
            ...
            for i, sample in tqdm(enumerate(samples), desc="Processing questions", total=len(samples)):
                try:
                    ...
                    if video_source == "EpicKitchens":
                        ...
                        video_id = sample["video_id"]
                        video_path = os.path.join(args.ego4d_video_dir, video_id)
                        print(video_path)
