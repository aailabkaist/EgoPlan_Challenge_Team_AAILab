...
from src.video_llama.video_llama.datasets.datasets.egoplan_video_instruct_dataset import (
    ...
    create_question_for_egoplan_task_planning_add_narr
)
...
@torch.no_grad()
def predict_choice(model, vis_processor, model_type, sample, return_loss=True, subset_name="Ego4D"):
    ...
    add_narr = sample["add_narr"]
    question = create_question_for_egoplan_task_planning_add_narr(task_goal, narr=add_narr, mode='eval')
...
def build(config_path = "src/video_llama/eval_configs/egoplan_video_llama_eval.yaml", model_config=None, epoch=0):
    if model_config is not None:
        config_path = f'src/video_llama/eval_configs/egoplan_video_llama_eval_{model_config}.yaml'
    args = Arguments(cfg_path=config_path,
                     model_type="llama_v2")
    cfg = Config(args)
    cfg.model_cfg.ckpt = cfg.model_cfg.ckpt[:cfg.model_cfg.ckpt.find('checkpoint_')+len('checkpoint_')] + f'{epoch}.pth'
    cfg.model_cfg.llm_lora_config.lora_ckpt = cfg.model_cfg.llm_lora_config.lora_ckpt[:cfg.model_cfg.llm_lora_config.lora_ckpt.find('lora_weights_')+len('lora_weights_')] + f'{epoch}'
