...
def load_video(sample, n_frms=MAX_INT, n_actions=N_ACTIONS, sampling="uniform", negative=False):
    # frame_idx starts with 1 in epic-kitchens dataset
    if negative: # random video sampling
        ...
        most_recent_actions_metadata = list(
            filter(lambda item: item['stop_frame'] <= current_observation_frame_idx,
                task_progress_metadata))[-n_actions:] # fine until now
        clips = []
        for i, action_metadata in enumerate(most_recent_actions_metadata):
            ...
            if i+1 < len(most_recent_actions_metadata):
                ...
                # print(stop_frame_idx, start_frame_idx)
            else:
        ...
        neg_current_observation_frame_idx = random.randint(1, current_observation_frame_idx-1)
        image_path = os.path.join(video_rgb_frame_dir, f"frame_{str(neg_current_observation_frame_idx).zfill(10)}.jpg")
        ...
        for i, action_metadata in enumerate(most_recent_actions_metadata):
            ...
            try:
                ...
                print("Except",vlen, n_frms, start_frame_idx, stop_frame_idx, action_metadata["start_frame"], action_metadata["stop_frame"], sample['sample_id'], sample['video_id'])
                raise e
            start, end = 0, vlen
            if sampling == "uniform":
                indices = np.arange(start, end, vlen / n_frms).astype(int).tolist()
...
class EgoplanVideoTrainProcessor(EgoplanVideoBaseProcessor):
    ...
    def __call__(self, sample, negative=False):
        """
        Args:
            clip (torch.tensor): Video clip to be cropped. Size is (C, T, H, W)
        Returns:
...
@registry.register_processor("egoplan_video_train_flip")
class EgoplanVideoTrainProcessor(EgoplanVideoBaseProcessor):
    def __init__(
        self,
        image_size=384,
        ...
        self.synchronized_flip = transforms_video.SynchronizedRandomHorizontalFlip()
    ...
    def __call__(self, sample):
        ...
        image, clips = self.synchronized_flip(image, clips)
