...
# Evaluation start, use 1 GPU.
CUDA_VISIBLE_DEVICES=${DEVICE} python -u eval_multiple_choice_eval.py \
--model ${MODEL_NAME} \
--epic_kitchens_rgb_frame_dir Your EpicKitchens dataset path \
--ego4d_video_dir Your Ego4D dataset path \
--model_config ${CONFIG} \
--epoch ${EPOCH} \
 2>&1 | tee -a logs_eval/eval_multiple_choice_${CONFIG}_RAG_${RAG}_EPOCH_${EPOCH}.log